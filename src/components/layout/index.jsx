...
// Interview removed as requested
const NAV = [
...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-6 h-14 border-b border-[#1e2d47] bg-[#050810]/90 backdrop-blur-md">
      ...
      {user ? (
        <div className="flex items-center gap-2 sm:gap-3">
          {trail && (
            <span className="hidden sm:flex items-center gap-1.5 text-xs text-[#8899bb] bg-[#111827] border border-[#1e2d47] rounded-lg px-2.5 py-1.5">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/login')} className="px-3 py-1.5 rounded-lg border border-[#1e2d47] text-[#8899bb] text-sm hover:bg-[#111827] hover:text-white transition-all">Entrar</button>
          <button onClick={() => navigate('/register')} className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition-all shadow-[0_0_16px_rgba(59,130,246,0.25)]">Começar</button>
        </div>
      )}
    </nav>
  )
  ...
  return (
    <aside className="sidebar-wrap w-52 min-w-[208px] border-r border-[#1e2d47] bg-[#090d18] flex flex-col overflow-y-auto">
      ...
      <div className="p-2 border-t border-[#1e2d47]">
        <button onClick={logout} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm text-[#8899bb] hover:bg-red-500/10 hover:text-red-400 transition-all w-full text-left">
  ...
  return (
    <nav className="bottom-nav">
      {NAV_MOBILE.map(item => {
        ...
        return (
          <button key={item.path} onClick={() => navigate(item.path)}
            className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-xl transition-all ${active ? 'text-blue-400' : 'text-[#4a5980]'}`}>
